...
public sealed class PatientsController : ApiController
{
    ...
    [HttpPost]
    public async Task<IActionResult> DeleteById(DeletePatientsByIdCommand request, CancellationToken cancellationToken)
    {
        var response = await _mediator.Send(request, cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Update(UpdatePatientsCommand request, CancellationToken cancellationToken)
    {
        var response = await _mediator.Send(request, cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
